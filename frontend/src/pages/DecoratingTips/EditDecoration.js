...
const EditDecoration = () => {
  ...
  const { id } = useParams(); // Get the tip ID from the URL
  ...
  // Fetch the existing tip data on mount
  useEffect(() => {
    fetch(`http://localhost:8080/api/decoration-tips/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setFormData({
          title: data.title,
          ...
          media: data.mediaUrls[0] || '', // Assuming only one media URL for simplicity
          author: data.author,
  ...
  const handleSubmit = async (e) => {
    ...
    const tipData = {
      ...
      // createdAt is not updated (handled by backend)
    };
    try {
      const response = await fetch(`http://localhost:8080/api/decoration-tips/${id}`, {
        method: 'PUT',
  ...
  return (
    <div className="create-decoration-container">
      ...
      <h1 className="form-header">Edit Decorating Tip</h1>
        ...
        <div className="form-section">
          ...
          <div className="mb-3">
            ...
            <small className="form-text text-muted">
              Current: {formData.media || 'None'} | Max 30 seconds for videos.
            </small>
          </div>
        </div>
